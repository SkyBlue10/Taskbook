    ...
    : QWidget(parent)
{
    ...
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE"); //добавл€ем базу
    db.setDatabaseName("./data/tasks.sqlite"); //ставим им€ базе (путь к бд)

    loadData(); //загрузка данных из Ѕƒ 
}
...
void TaskBook::reindexing()
{
    ...
    db.open();
    int i = 0;
    for (auto iter = tasks.begin(); iter != tasks.end(); iter++) //мен€ем значение тасков в Ѕƒ
    {
...
TaskBook::~TaskBook()
{
    ...
    reindexing(); //переназначение индексов внутри Ѕƒ
...
void TaskBook::addFromComplete(Task* for_all, Task* for_category, QString endDate) //---------------------------------------------------
{
    ...
    if (for_category == nullptr) {
        ...
        for (int i = 0; i < ui.categories->count(); i++)
        {
            if (ui.categories->widget(i)->objectName() == for_category->getCategory()) { //проверка наличи€ категории задачи
                presenceOfCategory = true;
                LogSystem::Write("Category " + for_all->getCategory().toStdString() + " does not exist");
                break;
            }
        }
        if (!presenceOfCategory)  //если категори€ по какой то причине отсутствует
        {
...
void TaskBook::onTabCloseRequested(int index)
{
    ...
    if (widget != nullptr) {
        std::map<int, Task*> temp; //временный костыль, который хотелось бы в будущем убрать или хот€ бы упростить алгоритм
